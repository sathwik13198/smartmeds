...
export class DatabaseStorage implements IStorage {
  ...
  async verifySession(sessionId: string): Promise<any> {
    return new Promise((resolve, reject) => {
      this.sessionStore.get(sessionId, (err: Error | null, session?: Session & SessionData | null) => {
        if (err) {
        ...
        if (!(session as CustomSession).userId) {
          resolve(null);
          return;
        }
        
        this.getUser((session as CustomSession).userId!).then(resolve).catch(reject);
      });
    });
  }
